...
object LowerBound {
  ...
  val x: List[Product with BoundObject[_ >: Village with Town with City <: Bound] with Serializable] = List.apply[Product with BoundObject[_ >: Village with Town with City <: Bound] with Serializable](A, B, C)
}