...
import utils.{CompilerService, SyntheticHelper}
...
class ExplicitImplicitsRule(g: ScalafixGlobal) extends SemanticRule("ExplicitImplicits") {
  ...
  override def withConfiguration(config: Configuration): Configured[Rule] = {
    if (config.scalacClasspath.isEmpty) {
      ...
      global match {
        case Success(settings) => Configured.ok(new ExplicitImplicitsRule(new ScalafixGlobal(settings, new StoreReporter, Map())))
