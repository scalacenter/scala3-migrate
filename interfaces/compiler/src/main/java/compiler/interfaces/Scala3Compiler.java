...
public class Scala3Compiler {
  ...
  private Function1<CompilationUnit, SourceFile> toSourceFile2 = new AbstractFunction1<CompilationUnit, SourceFile>() {
    public SourceFile apply(CompilationUnit unit) {
      AbstractFile abstractFile = AbstractFile$.MODULE$.getFile(unit.name);
  ...
  public void compileWithRewrite(List<CompilationUnit> units) throws CompilationException {
    List<SourceFile> sources = units.map(toSourceFile2);
    Reporter reporter = new QuietReporter();
    Context freshContext = context.fresh().setReporter(reporter);
    Run run = compiler.newRun(freshContext);
    run.compileSources(sources);
