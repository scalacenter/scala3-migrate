...
class LibToMigrate() extends AnyFunSuiteLike with DiffAssertions {
  ...
  val zioTests: Lib213                    = Lib213.from("dev.zio:zio-test:1.0.0-RC20", binary, false).get
  val zioTestsNonExistingRevision: Lib213 = Lib213.from("dev.zio:zio-test:2.0.5", binary, false).get
